...
from fest_friend_server import app
from fest_friend_server.server import (
    Database,
    get_db,
    get_id,
    unsigned_short,
...
def client():
    ...
    test_db = Database(
        lambda x: x, lambda x, y: x == y, {55: ["alice"]}, {55: "password55"}
    )
...
def test_generate_id():
    assert 35000 < generate_group_id(GroupIDs())


def test_join_group_ok(client: TestClient):
    res = client.post(
        "/join", json={"username": "john", "password": "password", "groupID": 55}
    )
...
def test_join_group_not_found(client: TestClient):
    res = client.post(
        "/join", json={"username": "john", "password": "password", "groupID": 65}
    )
...
def test_join_group_unauthorized(client: TestClient):
    res = client.post(
        "/join", json={"username": "john", "password": "pswd", "groupID": 55}
    )
...
def test_join_group_conflict(client: TestClient):
    res = client.post(
        "/join", json={"username": "alice", "password": "password", "groupID": 55}
    )
